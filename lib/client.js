/**
 * 明眸 VisionBridge —— 浏览器半边：设置卡片 + 模型页 stub 隐藏。
 *
 * 1. 在「设置」页注册一张「明眸 VisionBridge（视觉桥）」卡片
 *    （settings.section slot），字段读写主机 settings 命名空间 `vision-bridge`，
 *    保存即写 settings.yaml、实时生效（applies: live）。
 * 2. 隐藏「设置 → 模型」页那张由可配置提供方注册产生的 stub 卡片
 *    （该注册是让命名空间可写的必要机制，rc.6 没有 hidden 字段，
 *    只能像 zh_pro 一样用 DOM 观察器精确隐藏，中文界面生效）。
 *
 * 经典脚本（非 ESM），遵循客户端模块系统约定：
 * window.__ModuleLoader__.load({ id, factory })，工厂返回的 exports
 * 提供 inject 与 apply。
 */

window.__ModuleLoader__.load({
  id: 'dsh-mingmu',
  factory: function (require) {
    var module = { exports: {} }
    var exports = module.exports
    Object.defineProperty(exports, Symbol.toStringTag, { value: 'Module' })
    var React = require('react')

    var NS = 'vision-bridge'
    var CARD_ID = 'dsh-vision-bridge-settings'
    var I18N_NS = 'dsh-vision-bridge-settings'
    var PROVIDER_NAME = '明眸 VisionBridge（视觉桥）'

    var I18N_ZH = {
      nav: '明眸 VisionBridge（视觉桥）',
      desc: '自动判断当前模型能否看图：能看就原图直传（原生视觉，不降级）；不能看则先调视觉模型识别，把识别文字喂回主模型继续作答，全程无感。',
      enabled: '启用',
      enabledDesc: '总开关',
      providers: '生效的主模型 provider',
      providersDesc: '逗号分隔；留空 = 全部',
      visionBaseURL: '视觉 API 端点',
      visionBaseURLDesc: '视觉模型供应商地址',
      visionModel: '首选识别模型',
      visionModelDesc: '先用它识别；失败或结果为空时自动换备选',
      visionModelUpgrade: '备选识别模型',
      visionModelUpgradeDesc: '首选识别失败/为空时自动切换；留空 = 不启用备选',
      visionModels: '强制视觉模型白名单',
      visionModelsDesc: 'dsh 未声明视觉能力的模型填在这里（如 MiniMax/MiniMax-M3），逗号分隔',
      strategyHint: '策略：自动升级（首选失败 → 备选）',
      apiKeyEnv: 'API Key 引用',
      apiKeyEnvDesc: '环境变量名（key 留在凭据文件）',
      keyLabel: '视觉 API Key',
      keyDesc: '直接粘贴你的视觉服务 Key（写入 dsh 凭据库，不落盘到配置）',
      keyPlaceholder: 'sk-…',
      saveKey: '保存 Key',
      keySaved: 'Key 已保存 ✓',
      keyConfigured: '已配置 ✓',
      keyMissing: '未配置（请粘贴并保存）',
      keyLoading: '检测中…',
      keyError: '状态读取失败',
      failureMode: '失败策略',
      failureModeDesc: 'annotate 标注 / error 严格报错',
      maxTokens: '识别最大 token',
      timeoutMs: '识别超时（毫秒）',
      prompt: '识别提示词',
      promptDesc: '完整转录文字/布局/数据，只输出识别结果',
      save: '保存',
      saved: '已保存 ✓',
      pending: '设置加载中…',
    }
    var I18N_EN = {
      nav: 'VisionBridge',
      desc: 'Checks whether the current model can see images: vision-capable models get the original image natively; text-only models get an automatic vision description fed back as text — seamless.',
      enabled: 'Enabled',
      enabledDesc: 'Master switch',
      providers: 'Main-model providers',
      providersDesc: 'Comma separated; empty = all',
      visionBaseURL: 'Vision API endpoint',
      visionBaseURLDesc: 'Vision provider base URL',
      visionModel: 'Primary vision model',
      visionModelDesc: 'Used first; falls back automatically on failure/empty',
      visionModelUpgrade: 'Backup vision model',
      visionModelUpgradeDesc: 'Auto fallback; empty = disabled',
      visionModels: 'Force vision model allowlist',
      visionModelsDesc: 'Models not marked as vision by dsh (e.g. MiniMax/MiniMax-M3); comma separated',
      strategyHint: 'Strategy: auto-upgrade (primary → backup)',
      apiKeyEnv: 'API key env ref',
      apiKeyEnvDesc: 'Env var name (key stays in credentials file)',
      keyLabel: 'Vision API key',
      keyDesc: 'Paste your vision provider key (stored in dsh credentials, not config)',
      keyPlaceholder: 'sk-…',
      saveKey: 'Save key',
      keySaved: 'Key saved ✓',
      keyConfigured: 'Configured ✓',
      keyMissing: 'Missing (paste and save)',
      keyLoading: 'Checking…',
      keyError: 'Status read failed',
      failureMode: 'Failure mode',
      failureModeDesc: 'annotate / error',
      maxTokens: 'Max tokens',
      timeoutMs: 'Timeout (ms)',
      prompt: 'Vision prompt',
      promptDesc: 'Transcribe text/layout/data; output only the result',
      save: 'Save',
      saved: 'Saved ✓',
      pending: 'Loading settings…',
    }

    var boundScope = null
    var scopeListeners = []
    var scopeStore = {
    pendingSnapshot: Object.freeze({ status: 'pending' }),
      getSnapshot: function () {
        return boundScope === null ? scopeStore.pendingSnapshot : boundScope.getSnapshot()
      },
      subscribe: function (listener) {
        scopeListeners.push(listener)
        var unsub = null
        if (boundScope !== null && typeof boundScope.subscribe === 'function') {
          unsub = boundScope.subscribe(listener)
        }
        return function () {
          var at = scopeListeners.indexOf(listener)
          if (at !== -1) scopeListeners.splice(at, 1)
          if (unsub !== null) unsub()
        }
      },
    }

    // 全部使用官方 alias token，自动适配暗色/亮色主题
    var rowStyle = { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px', padding: '8px 0', borderBottom: '1px solid var(--dsw-alias-border-l2)' }
    var labelStyle = { fontSize: 14, lineHeight: '20px', color: 'var(--dsw-alias-label-primary)' }
    var descStyle = { fontSize: 12, lineHeight: '18px', color: 'var(--dsw-alias-label-tertiary)' }
    var inputStyle = {
      height: '34px',
      width: '260px',
      padding: '0 12px',
      borderRadius: '8px',
      border: '1px solid var(--dsw-alias-border-l2)',
      background: 'var(--dsw-alias-bg-layer-3)',
      color: 'var(--dsw-alias-label-primary)',
      font: 'inherit',
      fontSize: 13,
      lineHeight: '1.5',
    }
    var buttonStyle = {
      padding: '6px 16px',
      borderRadius: '8px',
      border: '1px solid var(--dsw-alias-brand-primary)',
      background: 'transparent',
      color: 'var(--dsw-alias-brand-text)',
      cursor: 'pointer',
      font: 'inherit',
    }
    var eyeIcon = React.createElement('svg', {
      width: 20,
      height: 20,
      viewBox: '0 0 24 24',
      fill: 'none',
      stroke: 'currentColor',
      strokeWidth: 1.8,
      strokeLinecap: 'round',
      strokeLinejoin: 'round',
      style: { color: 'var(--dsw-alias-label-secondary)', flex: 'none' },
    },
      React.createElement('path', { d: 'M2 12s3.5-7 10-7 10 7 10 7-3.5 7-10 7S2 12 2 12z' }),
      React.createElement('circle', { cx: 12, cy: 12, r: 3 }),
      React.createElement('circle', { cx: 12, cy: 12, r: 0.8, fill: 'currentColor', stroke: 'none' }))

    function fieldRow(key, label, desc, control) {
      return React.createElement('div', { key: key, style: rowStyle },
        React.createElement('div', { style: { minWidth: 0, flex: '1' } },
          React.createElement('div', { style: labelStyle }, label),
          React.createElement('div', { style: descStyle }, desc)),
        React.createElement('div', { style: { flex: 'none' } }, control))
    }

    var VisionBridgeSection = function (props) {
      var t = props.t
      var snapshot = React.useSyncExternalStore(scopeStore.subscribe, scopeStore.getSnapshot)
      var ready = snapshot !== null && snapshot.status === 'ready' && snapshot.value !== null && typeof snapshot.value === 'object'
      var value = ready ? snapshot.value : {}
      var draftState = React.useState(null)
      var draft = draftState[0]
      var setDraft = draftState[1]
      var savedState = React.useState(false)
      var saved = savedState[0]
      var setSaved = savedState[1]
      var keyState = React.useState('')
      var keyInput = keyState[0]
      var setKeyInput = keyState[1]
      var keyStatusState = React.useState({ state: 'loading' })
      var keyStatus = keyStatusState[0]
      var setKeyStatus = keyStatusState[1]
      var keySavedState = React.useState(false)
      var keySaved = keySavedState[0]
      var setKeySaved = keySavedState[1]

      var api = props.api
      var current = draft !== null ? draft : value
      var keyEnv = typeof current.apiKeyEnv === 'string' && current.apiKeyEnv.length > 0 ? current.apiKeyEnv : 'SILICONFLOW_API_KEY'

      var refreshKeyStatus = function () {
        if (api === null || api === undefined || api.credentials === undefined || typeof api.credentials.describe !== 'function') return
        setKeyStatus({ state: 'loading' })
        api.credentials.describe({ refs: [keyEnv] }).then(function (resp) {
          if (!resp.result.ok) { setKeyStatus({ state: 'error' }); return }
          var info = resp.result.value.credentials[keyEnv]
          setKeyStatus(info !== undefined
            ? { state: 'done', configured: info.configured === true }
            : { state: 'done', configured: false })
        }).catch(function () { setKeyStatus({ state: 'error' }) })
      }

      React.useEffect(function () {
        refreshKeyStatus()
        // 依赖 keyEnv，切换供应商时自动重新检测
      }, [keyEnv])

      var saveKey = function () {
        if (api === null || api === undefined || api.credentials === undefined || typeof api.credentials.set !== 'function') return
        if (keyInput.trim().length === 0) return
        api.credentials.set({ ref: keyEnv, value: keyInput.trim() }).then(function (resp) {
          if (resp.result.ok) {
            setKeySaved(true)
            setKeyInput('')
            refreshKeyStatus()
            setTimeout(function () { setKeySaved(false) }, 2000)
          }
        })
      }

      var update = function (key, next) {
        setSaved(false)
        setDraft(Object.assign({}, current, { [key]: next }))
      }
      var save = function () {
        if (boundScope === null || typeof boundScope.set !== 'function') return
        var patch = Object.assign({}, value, current)
        // providers 在 UI 中是逗号分隔字符串，保存前拆成数组
        if (typeof patch.providers === 'string') {
          patch.providers = patch.providers
            .split(',')
            .map(function (s) { return s.trim() })
            .filter(function (s) { return s.length > 0 })
        } else if (patch.providers === undefined || patch.providers === null) {
          patch.providers = []
        }
        Object.keys(patch).forEach(function (key) {
          try { void boundScope.set(key, patch[key]) } catch (e) { /* 单字段失败不影响其余 */ }
        })
        setSaved(true)
        setTimeout(function () { setSaved(false) }, 2000)
      }

      var text = function (key) {
        return React.createElement('input', {
          style: inputStyle,
          value: typeof current[key] === 'string' ? current[key] : '',
          onChange: function (e) { update(key, e.target.value) },
        })
      }
      var arrayText = function (key) {
        var value = current[key]
        var display = Array.isArray(value) ? value.join(', ') : (typeof value === 'string' ? value : '')
        return React.createElement('input', {
          style: inputStyle,
          value: display,
          placeholder: 'provider1, provider2',
          onChange: function (e) { update(key, e.target.value) },
        })
      }
      var number = function (key) {
        return React.createElement('input', {
          type: 'number',
          style: inputStyle,
          value: typeof current[key] === 'number' ? current[key] : 0,
          onChange: function (e) { update(key, Number(e.target.value)) },
        })
      }
      var toggle = function (key) {
        return React.createElement('input', {
          type: 'checkbox',
          checked: current[key] === true,
          onChange: function (e) { update(key, e.target.checked) },
        })
      }
      var select = function (key, options) {
        return React.createElement('select', {
          style: inputStyle,
          value: current[key] || options[0],
          onChange: function (e) { update(key, e.target.value) },
        }, options.map(function (opt) {
          return React.createElement('option', { key: opt, value: opt }, opt)
        }))
      }
      var keyStatusText = keyStatus.state === 'loading' ? t('keyLoading')
        : keyStatus.state === 'error' ? t('keyError')
          : keyStatus.configured === true ? t('keyConfigured') : t('keyMissing')

      if (!ready) {
        return React.createElement('div', { style: { padding: '16px 0', color: 'var(--dsw-alias-label-tertiary, #666)' } }, t('pending'))
      }

      return React.createElement('div', { style: { padding: '4px 0' } },
        React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: '8px', padding: '4px 0' } },
          eyeIcon,
          React.createElement('p', { style: descStyle }, t('desc'))),
        React.createElement('p', { style: descStyle }, t('strategyHint')),
        fieldRow('enabled', t('enabled'), t('enabledDesc'), toggle('enabled')),
        fieldRow('providers', t('providers'), t('providersDesc'), arrayText('providers')),
        fieldRow('visionBaseURL', t('visionBaseURL'), t('visionBaseURLDesc'), text('visionBaseURL')),
        fieldRow('visionModel', t('visionModel'), t('visionModelDesc'), text('visionModel')),
        fieldRow('visionModelUpgrade', t('visionModelUpgrade'), t('visionModelUpgradeDesc'), text('visionModelUpgrade')),
        fieldRow('visionModels', t('visionModels'), t('visionModelsDesc'), arrayText('visionModels')),
        fieldRow('apiKeyEnv', t('apiKeyEnv'), t('apiKeyEnvDesc'), text('apiKeyEnv')),
        fieldRow('apikey', t('keyLabel'), keyStatusText + ' · ' + t('keyDesc'),
          React.createElement('div', { style: { display: 'flex', gap: '6px', alignItems: 'center' } },
            React.createElement('input', {
              type: 'password',
              placeholder: t('keyPlaceholder'),
              style: Object.assign({}, inputStyle, { width: '180px' }),
              value: keyInput,
              onChange: function (e) { setKeyInput(e.target.value) },
            }),
            React.createElement('button', { style: buttonStyle, onClick: saveKey }, keySaved ? t('keySaved') : t('saveKey')))),
        fieldRow('failureMode', t('failureMode'), t('failureModeDesc'), select('failureMode', ['annotate', 'error'])),
        fieldRow('maxTokens', t('maxTokens'), '', number('maxTokens')),
        fieldRow('timeoutMs', t('timeoutMs'), '', number('timeoutMs')),
        fieldRow('prompt', t('prompt'), t('promptDesc'),
          React.createElement('textarea', {
            style: Object.assign({}, inputStyle, { height: 'auto', width: '320px', minHeight: '80px', padding: '8px 12px', resize: 'vertical' }),
            value: typeof current.prompt === 'string' ? current.prompt : '',
            onChange: function (e) { update('prompt', e.target.value) },
          })),
        React.createElement('div', { style: { padding: '12px 0' } },
          React.createElement('button', {
            style: buttonStyle,
            onClick: save,
          }, saved ? t('saved') : t('save'))))
    }

    function apply(ctx) {
      var api = ctx.get('connection') !== undefined && ctx.get('connection') !== null ? ctx.get('connection').api : null
      if (ctx.slots !== undefined && typeof ctx.slots.inject === 'function') {
        if (ctx.locale !== undefined && typeof ctx.locale.register === 'function') {
          ctx.effect(function () {
            ctx.locale.register(I18N_NS, { zh: I18N_ZH, en: I18N_EN })
          }, 'dsh-vision-bridge: settings dictionaries')
        }
        var t = (ctx.locale !== undefined && typeof ctx.locale.bind === 'function')
          ? ctx.locale.bind(I18N_NS)
          : function (key) { return I18N_ZH[key] || key }
        ctx.slots.inject('settings.section', function () {
          return ctx.slots.register({
            name: 'settings.section',
            id: CARD_ID,
            order: 51,
            label: function () { return t('nav') },
            locale: I18N_NS,
          }, function () {
            return React.createElement(VisionBridgeSection, { t: t, api: api })
          })
        })
      }

      // 直接注入 settingsScope（官方推荐写法），绑定 vision-bridge 命名空间
      var binder = ctx.get('settingsScope')
      if (binder !== undefined && binder !== null && typeof binder.bind === 'function') {
        var scope = binder.bind({ namespace: NS })
        boundScope = scope
        if (typeof scope.subscribe === 'function') {
          var unsub = scope.subscribe(function () {
            for (var i = 0; i < scopeListeners.length; i += 1) scopeListeners[i]()
          })
          ctx.effect(function () {
            return function () {
              if (boundScope === scope) boundScope = null
              if (typeof unsub === 'function') unsub()
            }
          }, 'dsh-vision-bridge: settings scope')
        }
      }

      // 隐藏「设置 → 模型」页里由可配置提供方注册产生的 stub 卡片（中文界面）。
      ctx.effect(function () {
        if (typeof document === 'undefined' || typeof MutationObserver === 'undefined') return
        var hideKey = 'data-dsh-vision-bridge-hide-provider'
        var hide = function (textNode) {
          if (textNode.data !== PROVIDER_NAME) return
          var el = textNode.parentElement
          for (var depth = 0; el !== null && depth < 6; depth += 1) {
            if (el.tagName === 'LI') {
              if (el.getAttribute(hideKey) === null) el.setAttribute(hideKey, '')
              el.style.setProperty('display', 'none', 'important')
              return
            }
            if (el.tagName === 'OPTION') {
              if (el.getAttribute(hideKey) === null) el.setAttribute(hideKey, '')
              el.hidden = true
              return
            }
            el = el.parentElement
          }
        }
        var run = function (root) {
          if (root.nodeType === 3) {
            hide(root)
            return
          }
          if (root.nodeType !== 1) return
          var child = root.firstChild
          while (child !== null) {
            run(child)
            child = child.nextSibling
          }
        }
        var observer = new MutationObserver(function () {
          observer.disconnect()
          try {
            if (document.body !== null) run(document.body)
          } finally {
            observer.observe(document.documentElement, { childList: true, subtree: true, characterData: true })
          }
        })
        var start = function () {
          if (document.body !== null) run(document.body)
          observer.observe(document.documentElement, { childList: true, subtree: true, characterData: true })
        }
        if (document.readyState === 'loading') {
          document.addEventListener('DOMContentLoaded', start, { once: true })
        } else {
          start()
        }
        return function () {
          observer.disconnect()
        }
      }, 'dsh-vision-bridge: hide models stub')
    }

    exports.inject = ['locale', 'slots', 'connection', 'settingsScope']
    exports.apply = apply
    return module.exports
  },
})

