...
import { isVisionModel, guessVisionModel } from './model-capabilities.mjs'

export const name = 'dsh-mingmu'
...
export async function apply(ctx, pluginConfig) {
  ...
  ctx.on(
    ...
    async ({ agent, signal }, next) => {
      ...
      try {
        const info = await originalResolveModelInfo(provider, model, signal)
        const heuristic = guessVisionModel(model)
        const userMatch = config.visionModels.some((entry) => {
          const trimmed = String(entry).trim()
          return trimmed === `${provider}/${model}` || trimmed === model
        })
        if (debug) console.log(`[vision-bridge][debug] real inputModalities=${JSON.stringify(info?.inputModalities)} heuristic=${heuristic} userMatch=${userMatch}`)
        // 用户白名单优先，其次是启发式；dsh 的 inputModalities 可能被 adapter patch
        // 污染（为了让 adapter 放行图片），所以不再单独作为不桥接的依据。
        if (userMatch) {
          if (debug) console.log('[vision-bridge][debug] user allowlist, skip bridge')
          return decision
        }
        if (heuristic) {
          if (debug) console.log('[vision-bridge][debug] heuristic vision, skip bridge')
          return decision
        }
        if (Array.isArray(info?.inputModalities) && info.inputModalities.includes('image')) {
          if (debug) console.log('[vision-bridge][debug] dsh reports vision but heuristic disagrees; trust heuristic and bridge')
        }
      } catch (error) {
