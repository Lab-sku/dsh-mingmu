 ...
 */
function patchAdapterModelOf(adapter, getVisionModels) {
  ...
  adapter.modelOf = function (snapshot, provider, model) {
    ...
    const { vision, reason } = isVisionModel(provider, model, resolved?.input, effectiveVisionModels(getVisionModels()))
    if (debug) console.log(`[vision-bridge][debug] modelOf ${full} vision=${vision} reason=${reason} rawInput=${JSON.stringify(resolved?.input)}`)
    if (vision && Array.isArray(resolved?.input) && !resolved.input.includes('image')) {
...
function installAdapterPatch(ctx, getVisionModels) {
  const llm = ctx.llm
  ...
  const patchAll = () => {
    ...
    for (const registration of regs) {
      patchAdapterModelOf(registration?.adapter, getVisionModels)
    }
  }
  patchAll()
